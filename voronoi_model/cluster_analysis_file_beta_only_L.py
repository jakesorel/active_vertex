...
def get_radial_profile_type(x_save,mult,L,c_types,res):
    x_range = (np.arange(res)+0.5)/res*L
    X,Y = np.meshgrid(x_range,x_range,indexing="ij")
    dX = np.outer(X.ravel(), np.ones_like(X.ravel())) - np.outer(np.ones_like(X.ravel()), X.ravel())
    dY = np.outer(Y.ravel(), np.ones_like(Y.ravel())) - np.outer(np.ones_like(Y.ravel()), Y.ravel())
    ...
    radialprofile=  get_radial_profile_type_norm(X, Y, res, x_save[-1], L, c_types, Dround)
    return radialprofile,ds
...
if __name__ == "__main__":
    ...
    def do_analysis(Id):
        for i in range(int(sys.argv[3])):
            for run in range(4):
                try:
                    tri_save = np.load("from_unsorted_beta_only/tri_save/%d_%d_%d.npz" % (Id,i,run))["arr_0"]
                    tri_save = tri_save.reshape(tri_save.shape[0], -1, 3)
                    x_save = np.load("from_unsorted_beta_only/x_save/%d_%d_%d.npz" % (Id,i,run))["arr_0"]
                    x_save = x_save.reshape(x_save.shape[0], -1, 2)
                    c_types = np.load("from_unsorted_beta_only/c_types/%d_%d_%d.npz" % (Id,i,run))["arr_0"]
                    ...
                    radialprofile, ds = get_radial_profile_type(vor.x_save, 10, vor.L, vor.c_types, res=40)

                    np.savez_compressed("from_unsorted_beta_only/autocorr/%d_%d.npz" % (Id,i + run*int(sys.argv[3])), ds=ds,radialprofile=radialprofile)
