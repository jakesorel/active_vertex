...
if __name__ == "__main__":
    ...
    def do_analysis(Id):
        for i in range(int(sys.argv[3])):
            try:
                tri_save = np.load("tri_save/%d_%d.npz" % (Id,i))["arr_0"]
                tri_save = tri_save.reshape(tri_save.shape[0], -1, 3)
                x_save = np.load("x_save/%d_%d.npz" % (Id,i))["arr_0"]
                x_save = x_save.reshape(x_save.shape[0], -1, 2)
                c_types = np.load("c_types/%d_%d.npz" % (Id,i))["arr_0"]
                ...
                np.savez_compressed("analysis/%d_%d.npz" % (Id,i), n_islands=n_islands, n_bound=n_bound, L_star=L_star,
                                    mean_self=mean_self)
