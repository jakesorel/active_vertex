...
if __name__ == "__main__":
    ...
    def do_analysis(Id):
        tri_save = np.load("tri_save/%d.npz" % Id)["arr_0"]
        tri_save = tri_save.reshape(tri_save.shape[0], -1, 3)
        x_save = np.load("x_save/%d.npz" % Id)["arr_0"]
        x_save = x_save.reshape(x_save.shape[0], -1, 2)
        c_types = np.load("c_types/%d.npz" % Id)["arr_0"]
        ...
        np.savez_compressed("analysis/%d.npz" % Id, n_islands=n_islands, n_bound=n_bound, L_star=L_star,
                            mean_self=mean_self)
    ...
    for Id in range(int(sys.argv[1])):
        lazy_result = dask.delayed(do_analysis)(Id)
