...
if __name__ == "__main__":
    ...
    def do_analysis(Id):
        ...
        for i in range(int(sys.argv[3])):
            for run in range(2):
                try:
                    tri_save = np.load("from_sorted_v0_vary/tri_save/%d_%d_%d.npz" % (Id,i,run))["arr_0"]
                    tri_save = tri_save.reshape(tri_save.shape[0], -1, 3)
                    x_save = np.load("from_sorted_v0_vary/x_save/%d_%d_%d.npz" % (Id,i,run))["arr_0"]
                    x_save = x_save.reshape(x_save.shape[0], -1, 2)
                    c_types = np.load("from_sorted_v0_vary/c_types/%d_%d_%d.npz" % (Id,i,run))["arr_0"]
                    ...
                    np.savez_compressed("from_sorted_v0_vary/analysis/%d_%d.npz" % (Id,i+ int(sys.argv[3])*run), n_islands=n_islands, n_bound=n_bound,
                                            mean_self=mean_self)
