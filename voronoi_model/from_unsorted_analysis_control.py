...
ax.set(xlabel=r"$\tau$",ylabel=r"$log_{10} \ \beta_max$")
...
t = 50
TTm, BBm = np.meshgrid(tau_range, beta_max_range, indexing="ij")


nis = n_islands.sum(axis=3).mean(axis=2)

cmap = plt.cm.plasma(np.linspace(0,1,12))
fig, ax = plt.subplots(1,12,figsize=(12,1.5),sharey=True)
ax = ax.ravel()
for i in range(12):
    for j in range(12):
        ax[j].plot(nis[i,j],color=cmap[i])
        ax[j].plot(mean_n_islands[j],color="black",linestyle=":")
        ax[j].set_title("lgb = %.1f"%np.log10(beta_max_range[j]),fontsize=2)
fig.subplots_adjust(top=0.8, bottom=0.2, left=0.25, right=0.8)

fig.savefig("paper_plots/Fig5/line_plots.pdf",dpi=300)

print("log beta",np.log10(BBm[nis==nis.min()]),"tau",TTm[nis==nis.min()])

vor.t_span = np.arange(0,500,0.025)
vor.tau = tau_range[10]
vor.beta_max = beta_max_range[6]
vor.beta_min = 10**-2.5

beta = np.array([vor.beta_t(t) for t in vor.t_span])
plt.plot(vor.t_span,np.log10(beta))

vor.tau = tau_range[11]
vor.beta_max = beta_max_range[6]
vor.beta_min = 10**-2.5
...
ax = ax.ravel()
for i in range(12):
    for ji, j in enumerate(np.arange(0,12,2)):
        ax[ji].plot(np.linspace(0,499,100).astype(np.int64),pis[i,j],color=cmap[i])
        # ax[j].plot(mean_n_islands[j],color="black",linestyle=":")
