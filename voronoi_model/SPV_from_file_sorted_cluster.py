...
def run_simulation(X):
    ...
    if not path.exists("from_sorted/tri_save/%d_%d_%d.npz"%(Id,rep,run)):
        ...
        np.savez_compressed("from_sorted/tri_save/%d_%d_%d.npz"%(Id,rep,run),vor.tri_save.reshape(vor.n_t,3*vor.n_v))
        np.savez_compressed("from_sorted/x_save/%d_%d_%d.npz"%(Id,rep,run),vor.x_save.reshape(vor.n_t,2*vor.n_c))
        np.savez_compressed("from_sorted/c_types/%d_%d_%d.npz"%(Id,rep,run),vor.c_types)
