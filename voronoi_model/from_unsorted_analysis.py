...
ax.imshow(flipim(final_mean_n_islands),extent=extent,aspect=aspect,cmap=plt.cm.inferno)
sm = plt.cm.ScalarMappable(cmap=plt.cm.inferno, norm=plt.Normalize(vmax=final_mean_n_islands.max(), vmin=final_mean_n_islands.min()))
sm._A = []
...
fig, ax = plt.subplots(figsize=(3,2.5))
extent, aspect = make_extent(v0_range,np.log10(beta_range))
ax.imshow(flipim(final_mean_mean_self),extent=extent,aspect=aspect,cmap=cmap)
sm = plt.cm.ScalarMappable(cmap=cmap, norm=plt.Normalize(vmax=final_mean_mean_self.max(), vmin=final_mean_mean_self.min()))
sm._A = []
cl = plt.colorbar(sm, ax=ax, pad=0.05, fraction=0.085, aspect=10, orientation="vertical")
