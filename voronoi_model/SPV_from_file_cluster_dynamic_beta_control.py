...
def run_simulation(X):
    ...
    np.savez_compressed("from_unsorted_control/c_types_tau0/%d_%d_%d.npz"%(Id,rep,run),vor.c_types)
...
if __name__ == "__main__":
    ...
    make_directory("from_unsorted_control")
    make_directory("from_unsorted_control/x_save_tau0")
    make_directory("from_unsorted_control/c_types_tau0")
    make_directory("from_unsorted_control/tri_save_tau0")

    TT,BB = np.meshgrid(tau_range,beta_max_range,indexing="ij")
    tau,beta_max = TT.take(Id),BB.take(Id)
    for repn in range(rep):
        run_simulation((tau,beta_max,Id,repn,run))
